...
"""project_4_trial_2.py DEPRECATED - SECOND SHOT
...
T_G_WIDTH, T_G_HEIGHT, T_G_NUMCHANNELS = 100, 100, 3
CHUNK_SIZE = 256
BATCH_SIZE = 2
LEARNING_RATE = 0.001
EMBEDDING_SIZE = 300
...
def main(argv):
    ...
    if len(argv) < 2:
        print(
            "Usage: \n\t -learn <Train Anchors (TXT)> <Train Positives (TXT)> <Train Negatives "
            "(TXT)> <Val Anchors (TXT)> <Val Positives (TXT)> <Val Negatives (TXT)> <embedding "
            "size> <batch size> <num epochs> <output model prefix> \n\t -extract <Model Prefix> <Input Image List (TXT)> <Output File (TXT)> \n\t\tBuilds and scores a triplet-loss model "
        )
        return
...
def extract(argv):
    ...
    total_img_ch = int(np.ceil(total_img / float(CHUNK_SIZE)))

    with open(outfile, "w") as f_handle:

        for i in range(0, total_img_ch):
            imgs = t_read_image_list(imglist, i * CHUNK_SIZE, CHUNK_SIZE)
...
def learn(argv):
    ...
    if len(argv) < 10:
        print(
            "Usage: \n\t <Train Anchors (TXT)> <Train Positives (TXT)> <Train Negatives (TXT)> "
            "<Val Anchors (TXT)> <Val Positives (TXT)> <Val Negatives (TXT)> <embedding size> "
            "<batch size> <num epochs> <output model> \n\t\tLearns triplet-loss model"
        )
        return
    ...
    for e in range(0, numepochs):
        ...
        for t in range(0, total_t_ch):
            ...
            model.fit(
                createDataGen(
                    anchors_t, positives_t, negatives_t, Y_train, batch
                ),
                steps_per_epoch=len(Y_train) / batch,
                epochs=1,
                shuffle=False,
        ...
        for v in range(0, total_v_ch):
            ...
            curval = model.evaluate(
                [anchors_v, positives_v, negatives_v], Y_val, batch_size=batch
            )
